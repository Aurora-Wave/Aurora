"""
PlotStyleManager - Centralized styling management for plots.
...
class PlotStyleManager(QObject):
    ...
    def get_style_for_signal(
        self, signal_name: str, custom_style: Optional[PlotStyle] = None
    ) -> PlotStyle:
        """
    ...
    def update_min_height(self, signal_name, new_min: int):
        ...
        if signal_name == "*":
            ...
            for name in self.custom_styles:
                ...
                self.custom_styles[name] = replace(
                    current_style, min_plot_height=new_min
                )
            ...
            self.logger.debug(
                f"Updated minimum height for {signal_name} to {new_min}px"
            )
