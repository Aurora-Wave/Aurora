"""
CustomPlot - Advanced individual plot widget.
...
Features:
    ...
    - Centralized style & comment manager integration
    - Optional units display beside the signal name

Adapted for Aurora's session-based architecture.
"""
...
class NoScrollComboBox(QComboBox):
    """QComboBox that ignores wheel scrolling to avoid accidental changes."""
...
class CustomPlot(QWidget):
    """Self-contained plot with full integration hooks.
    ...
    Provides:
        ...
        - Dynamic signal switching
        - Context menus
        - Central style & comment manager integration
        - Units label (if known)
    """
    ...
    def __init__(
        ...
        # Get current style
        self.current_style = self.style_manager.get_style_for_signal(
            signal_name, custom_style
        )
    ...
    def configure_plot(self):
        ...
        self.style_manager.configure_plot_widget(
            self.plot_widget, self.signal_name, self.current_style
        )

        # Units mapping (fallback defaults when full metadata/trace is not cheaply available)
        units_defaults = {
            "hr_aurora": "bpm",
            "ECG": "mV",
            "FBP": "mmHg",
        ...
        unit = units_defaults.get(self.signal_name)
        left_label = f"{self.signal_name} ({unit})" if unit else self.signal_name
        ...
        self.logger.debug(
            f"CustomPlot {self.signal_name} minimum height set to: {self.min_height}px"
        )
        ...
        self.logger.debug(
            f"Plot configured for {self.signal_name} using PlotStyleManager"
        )
    ...
    def update_data(self, time_data: np.ndarray, y_data: np.ndarray):
        ...
        try:
            ...
            if hasattr(self, "curve") and self.curve is not None:
                ...
                pen = self.style_manager.create_plot_pen(
                    self.signal_name, self.custom_style
                )
                self.curve = self.plot_widget.plot(
                    time_data, y_data, pen=pen, name=self.signal_name
                )
    ...
    def refresh_min_height(self, new_min: int):
        ...
        self.logger.debug(
            f"Refreshed {self.signal_name} minimum height to: {self.min_height}px"
        )
