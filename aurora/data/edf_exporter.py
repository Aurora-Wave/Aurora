...
try:
    import mne  # Heavy dependency; retained per user requirement
except ImportError as _mne_err:
...
from aurora.core.signal import (
    Signal,
    HR_Gen_Signal,
)  # HR_Gen_Signal alias retro (hr_aurora)
...
class EDFExporter:
    ...
    def __init__(self, data_manager: DataManager):
        ...
        if mne is None:
            raise ImportError(
                "mne is required for EDF export. It must remain installed as per retained dependencies."
            ) from _mne_err  # type: ignore
        ...
        **hr_params,
        ...
        try:
            ...
            if file_path not in loaded_files:
                raise ValueError(
                    f"File not loaded in DataManager: {file_path}. Loaded files: {loaded_files}"
                )
            ...
            self.logger.info(
                f"Exporting {len(export_channels)} channels: {export_channels}"
            )
    ...
    def _load_signals(
        ...
        **hr_params,
        ...
        for channel_name in channels:
            try:
                # Load full signal
                if channel_name.upper() in ("HR_GEN", "HR_AURORA"):
                    signal = self.data_manager.get_trace(
                        file_path, channel_name, **hr_params
                    )
                else:
                ...
                self.logger.debug(
                    f"Loaded {channel_name}: {len(signal.data)} samples, fs={signal.fs}, units={normalized_units}"
                )
    ...
    def _resample_signals(
        self, signal_data: dict, channel_info: dict, target_fs: Optional[float]
    ) -> tuple:
        """
        ...
        if len(unique_fs) == 1:
            ...
            self.logger.debug(
                f"All signals have same fs: {unique_fs[0]}Hz, no resampling needed"
            )
            return signal_data, channel_info
        ...
        for channel_name, data in signal_data.items():
            ...
            if original_fs == target_fs:
                ...
                self.logger.debug(
                    f"Resampled {channel_name}: {len(data)} -> {len(resampled_data[channel_name])} samples"
                )
    ...
    def _create_mne_raw(
        ...
        for idx, name in enumerate(channel_names):
            ...
            self.logger.debug(
                f"Channel {name}: units='{units_str}' -> base={base_unit}, mul={unit_mul}"
            )

        # Create Raw object
        raw = mne.io.RawArray(data_matrix, info)

        # TODO: meas_date should reflect the original recording time (from .adicht metadata),
        # but it's unclear if that info is available. For now use current UTC time.
        raw.set_meas_date(dt.datetime.now(dt.timezone.utc))

        self.logger.debug(
            f"Created MNE Raw: {len(channel_names)} channels, {data_matrix.shape[1]} samples, {sfreq}Hz (all as 'misc' type)"
        )
        ...
        try:
            ...
            for comment in all_comments:
                ...
                if (
                    ...
                    and int(comment.comment_id) in excluded_comment_ids
                ):
                    excluded_count += 1
                    continue
            ...
            if filtered_comments:
                ...
                self.logger.info(
                    f"Added {len(filtered_comments)} annotations (excluded {excluded_count})"
                )
            else:
    ...
    def _export_raw_to_edf(
        self, raw: object, output_path: str, overwrite: bool
    ) -> None:
        """
    ...
    def get_export_info(
        self, file_path: str, channels: Optional[List[str]] = None
    ) -> dict:
        """
        ...
        hr_support = "ECG" in available_channels or "EKG" in available_channels
        return {
            "file_path": file_path,
            ...
            # Compatibilidad: se mantienen ambas claves mientras se migra UI
            "supports_hr_gen": hr_support,
            "supports_hr_aurora": hr_support,
