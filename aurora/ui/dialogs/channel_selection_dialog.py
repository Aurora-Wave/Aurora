...
class ChannelSelectionDialog(QDialog):
    ...
    def __init__(
        ...
        self.logger.info(
            f"ChannelSelectionDialog opened with {len(channel_names)} channels"
        )
    ...
    def init_ui(self, channel_names: List[str]):
        ...
        for name in channel_names:
            ...
            if (
                name.lower() in ("hr_gen", "hr_aurora")
                and name not in self.existing_channels
            ):
                display_name = f"{name} (will be generated)"
                checkbox = QCheckBox(display_name)
                checkbox.setToolTip(
                    "This channel will be generated using default parameters when selected"
                )
            else:
    ...
    def _apply_config_channels(self):
        ...
        try:
                ...
                if best_match:
                    ...
                    for checkbox in self.checkboxes:
                        ...
                        if " (will be generated)" in checkbox_name:
                            checkbox_name = checkbox_name.split(" (will be generated)")[
                                0
                            ]
            ...
            if not_found:
                self.logger.warning(
                    f"Config channels not found (no fuzzy match): {not_found}"
                )
                ...
                for missing in not_found:
                    ...
                    for available in available_channels:
                        ...
                        if (
                            missing_norm in available_norm
                            or available_norm in missing_norm
                            or any(
                                part in available_norm
                                for part in missing_norm.split("_")
                                if len(part) > 2
                            )
                        ):
