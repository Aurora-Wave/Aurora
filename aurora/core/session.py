...
class Session(QObject):
    ...
    def load_file(self, selected_channels: List[str] = None, config_file_path: Optional[str] = None) -> bool:
        ...
        try:
            ...
            self.logger.info(f"File exists - proceeding with DataManager.load_file()...")
            
            # Load file with DataManager
            try:
                self.logger.debug(f"Calling data_manager.load_file({self.file_path})...")
                load_result = self.data_manager.load_file(self.file_path)
                self.logger.info(f"data_manager.load_file() completed with result: {load_result}")
            except Exception as e:
                self.logger.error(f"Exception in data_manager.load_file(): {e}", exc_info=True)
                raise
            
            # Get available channels
            self.logger.debug(f"Getting available channels...")
            try:
                available_channels = self.data_manager.get_available_channels(self.file_path)
                self.logger.info(f"Available channels: {available_channels} (count: {len(available_channels) if available_channels else 0})")
            except Exception as e:
                self.logger.error(f"Exception getting available channels: {e}", exc_info=True)
                raise
            ...
            if not selected_channels:
                if config_file_path:
                    ...
                    self.logger.info(f"Loading channels from config file: {config_file_path}")
                    try:
                        from aurora.core.config_manager import load_channels_from_config_file
                        
                        config_channels = load_channels_from_config_file(config_file_path)
                        
                        if config_channels:
                            # Validate channels exist in available channels
                            valid_channels = [ch for ch in config_channels if ch in available_channels]
                            if valid_channels:
                                self.logger.info(f"Using {len(valid_channels)} channels from config file: {valid_channels}")
                                selected_channels = valid_channels
                            else:
                                self.logger.warning(f"No valid channels found in config file, showing dialog")
                        else:
                            self.logger.warning(f"No channels defined in config file, showing dialog")
                ...
                if not selected_channels:
                    ...
                    try:
                        from aurora.ui.dialogs.channel_selection_dialog import ChannelSelectionDialog
                        ...
                        self.logger.info(f"User selected {len(selected_by_user)} channels: {selected_by_user}")
                        selected_channels = selected_by_user
                        
                    except ImportError as e:
                        self.logger.warning(f"Could not import ChannelSelectionDialog: {e}")
                        selected_channels = available_channels
                    except Exception as e:
                        self.logger.error(f"Error showing channel selection dialog: {e}", exc_info=True)
                        selected_channels = available_channels
            ...
            # TODO: Initialize ChunkLoader
            # self.chunk_loader = ChunkLoader(self.data_manager)
    ...
    def close(self) -> None:
        try:
            # TODO: When chunk_loader is implemented
            # if self.chunk_loader and hasattr(self.chunk_loader, 'stop'):
            #     self.chunk_loader.stop()
            # self.chunk_loader = None
            
            # TODO: Clear DataManager cache for this file
