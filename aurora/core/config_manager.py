...
class ConfigManager:
    ...
    def __init__(self):
        ...
        # Window Setting
        self.window_settings: Dict[str, Any] = {
            "geometry": None,
            "maximized": False
        }
...
class ConfigManagerSystem:
    ...
    def _update_config_from_dict(self, data: Dict[str, Any]):
        ...
        # Simple fields
        self.config.default_chunk_size = data.get('default_chunk_size', self.config.default_chunk_size)
        self.config.default_visible_channels = data.get('default_visible_channels', self.config.default_visible_channels)
        self.config.export_format = data.get('export_format', self.config.export_format)
        self.config.last_file_path = data.get('last_file_path', self.config.last_file_path)
    ...
    def get_hr_validation_limits(self) -> tuple[float, float]:
        ...
        return (self.config.hr_generation["min_hr_bpm"], 
...
def load_channels_from_config_file(config_file_path: str) -> List[str]:
    ...
    try:
        ...
        logging.getLogger("aurora.core.ConfigManager").error(f"Error loading channels from {config_file_path}: {e}")
        return []