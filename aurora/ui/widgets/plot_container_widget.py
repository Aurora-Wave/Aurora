...
from PySide6.QtWidgets import (
    ...
    QLabel, QSpinBox, QPushButton, QFrame, QSlider, QSizePolicy, QApplication
)
...
class PlotContainerWidget(QWidget):
    ...
    def __init__(self, parent=None, tab_type: str = None):
        ...
        self.comment_markers: List[pg.InfiniteLine] = []  # Simple list of active markers
    ...
    def display_signals(self, data_manager, file_path: str, target_signals: List[str], hr_params: Dict = None):
        """
        ...
        try:
            ...
            self.logger.info(f"=== PlotContainer display_signals SUCCESS with {len(target_signals)} signals ===")
            return True
    ...
    def create_plots(self):
        ...
        for plot_index, signal_name in enumerate(self.target_signals):
            plot = CustomPlot(signal_name, available_signals=self.available_signals, 
                            plot_container=self, plot_index=plot_index)
    ...
    def add_plot(self, signal_name: str):
        ...
        if signal_name not in self.target_signals and signal_name in self.available_signals:
            ...
            if self.data_manager and self.file_path:
                try:
                    ...
                    if signal:
                        ...
                        end_sample = int(signal.fs * (self.start_time + self.chunk_size))
                        end_sample = min(end_sample, len(signal.data))
                        
                        if start_sample < len(signal.data):
                            # Extract data chunk
                            chunk_data = signal.data[start_sample:end_sample]
                            time_data = np.arange(len(chunk_data)) / signal.fs + self.start_time
                            
                            # Update plot
                            plot.update_data(time_data, chunk_data)
                            
                except Exception as e:
                    self.logger.error(f"Error loading data for new plot {signal_name}: {e}")
    ...
    def load_and_display_data(self):
        ...
        try:
            ...
            self.logger.debug(f"Loading data for time window: {start_time:.1f}s - {end_time:.1f}s")
            ...
            for plot in self.plots:
                ...
                try:
                    ...
                    if start_sample >= len(signal.data):
                        self.logger.warning(f"Start time beyond signal length for {signal_name}")
                        continue
                    ...
                    self.logger.debug(f"Updated {signal_name}: {len(chunk_data)} samples, {signal.fs}Hz")
                    
                except Exception as e:
                    self.logger.error(f"Error loading data for signal {signal_name}: {e}")
    ...
    def refresh_comment_display(self, comments_to_render):
        ...
        try:
            ...
            self.logger.debug(f"Rendered {len(self.comment_markers)} comment markers across {len(self.plots)} plots")
    ...
    def _create_comment_markers_for_all_plots(self, comment):
        ...
        try:
            ...
            # Prepare label text
            label_text = comment.text[:30] + "..." if len(comment.text) > 30 else comment.text
    ...
    def _update_splitter_size(self):
        ...
        self.logger.info(f"  - Individual minimumHeight(): {[plot.minimumHeight() for plot in self.plots]}")
        ...
        self.logger.info(f"  - Condition (total > viewport): {total_height > viewport_height}")
    ...
    def cleanup(self):
        ...
        try:
            ...
            try:
                self.style_manager.minHeightChanged.disconnect(self._on_global_height_changed)
