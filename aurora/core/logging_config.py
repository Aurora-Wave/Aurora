...
class AuroraLoggerConfig:
    ...
    def _get_application_root(cls) -> Path:
        ...
        if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
            ...
            # Development mode: find project root by looking for aurora folder
            current_file = Path(__file__)
            
            # Start from current file and go up to find aurora folder
            search_path = current_file.parent
            for _ in range(5):  # Limit search depth
                # Look for aurora folder structure
                aurora_main = search_path.parent / "aurora" / "main.py"
                if aurora_main.exists():
